...
void laserTower::loadTexture()
{
	...
	texture = *MainGra::getInstance()->textures.getTowerTexture(textureName);
	sprite.setTexture(texture);

	sf::Vector2u textureSize = texture.getSize();
	sprite.setTextureRect(sf::IntRect(0, 0, textureSize.x / 3, textureSize.y));

	laserT = MainGra::getInstance()->textures.getTexture("laser");

	sf::Texture *arrowDirT = MainGra::getInstance()->textures.getTexture("arrowDir");
...
{
	std::vector<wave*> & monsters = MainGra::getInstance()->getWaves();
...
{
	accumulator += Gra::getInstance()->getTimer();
...
void laserTower::shoot()
{
	...
	std::vector<wave*> & waves = MainGra::getInstance()->getWaves();
