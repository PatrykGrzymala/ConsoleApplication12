...
menuParticleSystem::menuParticleSystem()
{
	...
	for (int i = 0; i < cVertexesCount; ++i)
	{
		...
		cursorVertexes[i].position = (sf::Vector2f)sf::Mouse::getPosition(Gra::getInstance()->getWindow());
...
void menuParticleSystem::resetCursorParticle(int index)
{
	...
	newPointPos.x = (float)(rand() % WIDTH);
	newPointPos.y = (float)(rand() % HEIGHT);
	cursorVertexes[index].position = (sf::Vector2f)sf::Mouse::getPosition(Gra::getInstance()->getWindow());
...
{
	sf::Time elapsedTime = Gra::getInstance()->getTimer();
