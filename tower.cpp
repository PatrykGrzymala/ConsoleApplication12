...
tower::tower()
{
	...
	globalTimer = &Gra::getInstance()->getTimer();
}
...
bool tower::canBuy()
{
	if ((MainGra::getInstance()->getWallet()->isAfford(cost)) && (getShopTemplate()->isUnlocked()))
		return true;
...
bool tower::isAfford()
{
	if (MainGra::getInstance()->getWallet()->isAfford(cost))
		return true;
...
{
	int gold = MainGra::getInstance()->getWallet()->getGold();
...
void tower::sell()
{
	...
	MainGra::getInstance()->getWallet()->deposit(val);
