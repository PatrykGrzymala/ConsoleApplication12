...
towerManagament::towerManagament(workshop *Workshop)
{
	...
	{
		std::cout<<"Unable to open shopTexture";
		return;
	}
	...
	gameMap = MainGra::getInstance()->getMap();
...
void towerManagament::handleShop()
{
	...
	if (isTowerOnMouse())
	{
		...
		if ((Button::canClick()) && (sf::Mouse::isButtonPressed(sf::Mouse::Button::Left)) && (gameMap->canTowerBuild(currentTower)))
		{

			MainGra::getInstance()->getWallet()->pay(currentTower->cost);
